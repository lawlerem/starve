...
# test_that("C++ nngp",{
  ...
  expect_equal(report$one_pg_re, pg_re[2,1,1])
  expect_equal(report$one_tg_re, tg_re[3,2])
...
# test_that("C++ distribution",{
  ...
  # Normal
  expect_equal(report$ans[,1], dnorm(x[,1],mean[[1]],pars[[1,1]],log=TRUE))

  # Poisson
  expect_equal(report$ans[,2], dpois(x[,2],mean[[2]],log=TRUE))

  # Neg. Binomial
  # var = par*mean = mean + mu^2/size; 1/size = (par-1)*mean / mean^2; size = mean/(par-1)
  expect_equal(report$ans[,3], dnbinom(x[,3],mu=mean[[3]],size=mean[[3]]/(pars[[1,3]]-1),log=TRUE))

  # Bernoulli
  expect_equal(report$ans[,4], dbinom(x[,4],size=1,prob=mean[[4]],log=TRUE))

  # Gamma
  expect_equal(report$ans[,5], dgamma(x[,5],shape=(mean[[5]]/pars[[1,5]])^2,scale=pars[[1,5]]^2/mean[[5]],log=TRUE))

  # Log-normal
  expect_equal(report$ans[,6], dlnorm(x[,6], meanlog=mean[[6]], sdlog=pars[[1,6]], log=TRUE))

  # Binomial
  expect_equal(report$ans[,7], dbinom(x[,7],size=size[,7],prob=mean[[7]],log=TRUE))

  # atLeastOneBinomial
  expect_equal(report$ans[,8], ifelse(x[,8]==0,
...
# test_that("C++ family",{
  ...
  # Poisson
  expect_equal(report$ans[,2], dpois(x[,2],exp(mean[[2]]),log=TRUE))

  # Neg. Binomial
  # var = par*mean = mean + mu^2/size; 1/size = (par-1)*mean / mean^2; size = mean/(par-1)
  expect_equal(report$ans[,3], dnbinom(x[,3],mu=exp(mean[[3]]),size=exp(mean[[3]])/(pars[[1,3]]-1),log=TRUE))

  # Bernoulli
  expect_equal(report$ans[,4], dbinom(x[,4],size=1,prob=plogis(mean[[4]]),log=TRUE))

  # Gamma
  expect_equal(report$ans[,5], dgamma(x[,5],shape=(exp(mean[[5]])/pars[[1,5]])^2,scale=pars[[1,5]]^2/exp(mean[[5]]),log=TRUE))

  # Log-normal
  expect_equal(report$ans[,6], dlnorm(x[,6], meanlog=mean[[6]], sdlog=pars[[1,6]], log=TRUE))

  # Binomial
  expect_equal(report$ans[,7], dbinom(x[,7],size=size[,7],prob=plogis(mean[[7]]),log=TRUE))
