...
The spatial parameters describe the spatial covariance function.
...
The data used in the model holds the data supplied to 'prepare_staRVe_model'.
It also include predictions for the random effects (w), the response mean before applying the link function (linear), and the response mean after applying the link function (response).
...
- **distance_units**: units used for distance calculations
...
For a more detailed explanation of the directed acyclic graph, see section ###.



## (Advanced)

(INLA mesh, TMB object, graph details, pre-computed graphs, optimizer settings)
...
spatial_parameters(bird_model)["sd","fixed"]<- T
...
formula(bird_model)<- cnt~time(year)+mean(elev)

formula(bird_model)
fixed_effects(bird_model)
```

The model formula for including covariates such as the newly created 'elev' variable will be explained in section ###.
...
We first randomly sample a small number of points for predictions, and store those points as an sf data.frame.
The only part of the prediction locations used are the locations themselves; a year column is unnecessary.
The unique locations provided will predicted at for each year desired.
By default predictions will be made for every year spanned by the dataset, from the earliest year to the latest.
...
The output is a sf data.frame containing predictions for
...
To predict outside of the timespan of the original data, or to predict at specific years only you can supply specific years to the 'time' option.
...
The output is a list of rasters with one list element / raster for each year of predictions.
Each of those rasters has a layer for the same predictions as that point predictions give.
...
Predictions outside the timespan of the original data or at specific years only use the same 'time' option as the point predictions.
...
Once a staRVe model is created from the template data, the parameter values and any tweaks to the model (such as changing the covariance function, link function, response distribution, etc.) need to be changed.


```{r sim-new2}
time_parameters(sim_model)[c("mu","ar1","sd"),"par"]<- c(10,0.7,1)
spatial_parameters(sim_model)["sd","par"]<- 2
response_parameters(sim_model)["sd","par"]<- 0.5
sim_model<- staRVe_simulate(sim_model)
sim_model
```
...
```{r simulate-all,eval=F}
head(time_effects(sim_model)$w)
head(random_effects(sim_model)$w)
head(dat(sim_model)$cnt)
...
The only change needed to include covariates is to add a 'mean(...)' term to the model formula.
Inside of the 'mean(...)' term can be the right-hand side of any formula, with some exceptions.
The 'offset()' term is not recognized, and the '.' shorthand for 'all variables not otherwise in the formula' is not recognized.
In our example we include elev and temp by writing 'mean(elev+temp)'.
A corresponding formula to 'lm' without any spatial or temporal information would be
cnt~elev+temp'.
...
bird_model<- prepare_staRVe_model(
  cnt ~ time(year) + mean(I(abs(temp))+poly(elev,2)),
  bird_survey,
  distribution = "poisson",
  fit = T
)
...
```{r eval=F}
response ~ mean(...) + sample.size(...) + time(...) + space(...)
```

- **mean(...)**: Covariate effects can be specified as they would be in a typical call to 'lm' or 'glm'. Can be omitted from the formula if no covariates effects are desired.
...
- **space(...,nu=)**: The name of the covariance function should be given. If using the 'matern' covariance function, supplying a value to the nu option fixes the smoothness parameter nu at that value. Can be omitted from the formula, in which case the default exponential covariance function will be used.
